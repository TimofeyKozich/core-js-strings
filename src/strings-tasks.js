...
function countVowels(str) {
  const newStr = str.replace(/[aeiouy]/gi, '');
...
function isPalindrome(str) {
  const str1 = str.replace(/[^a-z]/gi, '').toLowerCase();
...
function reverseWords(str) {
  const arr = str.split(' ');
  const arrRevers = arr.map((value) => value.split('').reverse().join(''));
...
function getCardId(value) {
  ...
  const card2 = ['♣', '♦', '♥', '♠'];
  const firstIndex = card1.indexOf(value.slice(0, -1));
  const secondIndex = card2.indexOf(value.slice(-1));
  return firstIndex + secondIndex * 13;
}
